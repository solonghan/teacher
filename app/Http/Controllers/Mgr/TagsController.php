...
class TagsController extends Mgr
{
	...
	public function add(Request $request){
		if ($request->isMethod('post')) {
			...
			$res = Tags::updateOrCreate($formdata['tw']);

			$formdata['en']['parent_id'] = $res->id;
			$formdata['en']['lang'] = 'en';
			Tags::updateOrCreate($formdata['en']);
			if ($res) {
	...
	public function edit(Request $request, $id){
		...
		if ($request->isMethod('post')) {
			...
			$res = Tags::updateOrCreate(['id'=>$id], $formdata['tw']);
			Tags::updateOrCreate(['parent_id'=>$id, 'lang'=>'en'], $formdata['en']);
			if ($res) {
