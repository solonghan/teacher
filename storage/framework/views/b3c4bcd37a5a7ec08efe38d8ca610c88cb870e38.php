...
<?php $__env->startSection('content'); ?>
    ...
    <div class="row">
        <div class="col-12">
        <?php if($type=='search'): ?>
             <p>查詢條件  姓名:<?php echo e($search_name); ?>， 服務單位:<?php echo e($search_now_unit); ?> <?php echo e($have); ?>，職稱:<?php echo e($search_title); ?>，專門專長:<?php echo e($search_specialty); ?>，學術專長:<?php echo e($search_academic); ?>  ，最後異動時間:<?php echo e($last_date); ?></p>
    ...
    <script>
        $(document).ready(function(e){
            ...
            $(document).on('click', '.edit_department-item-btn', function(event) {
                ...
                location.href = '<?php echo e(env('APP_URL')); ?>/mgr/<?php echo e($controller??''); ?>/department_edit/' + id;
            });

            $(document).on('click', '.view_department-item-btn', function(event) {
                var id = $(this).closest('tr').data('id');
                location.href = '<?php echo e(env('APP_URL')); ?>/mgr/<?php echo e($controller??''); ?>/department_view/' + id;
            });

            $(document).on('click', '.academics-item-btn', function(event) {
                var id = $(this).closest('tr').data('id');
                location.href = '<?php echo e(env('APP_URL')); ?>/mgr/<?php echo e($controller??''); ?>/academics/' + id;
            });

            $(document).on('click', '.edit_academics-item-btn', function(event) {
                var id = $(this).closest('tr').data('id');
                location.href = '<?php echo e(env('APP_URL')); ?>/mgr/<?php echo e($controller??''); ?>/edit_academics/' + id;
            });

             $(document).on('click', '.specialty-item-btn', function(event) {
                var id = $(this).closest('tr').data('id');
                location.href = '<?php echo e(env('APP_URL')); ?>/mgr/<?php echo e($controller??''); ?>/specialty/' + id;
            });
             $(document).on('click', '.specialty_list-item-btn', function(event) {
                var id = $(this).closest('tr').data('id');
                location.href = '<?php echo e(env('APP_URL')); ?>/mgr/<?php echo e($controller??''); ?>/specialty_list/' + id;
            });

            $(document).on('click', '.academics_list-item-btn', function(event) {
                var id = $(this).closest('tr').data('id');
                location.href = '<?php echo e(env('APP_URL')); ?>/mgr/<?php echo e($controller??''); ?>/academics_list/' + id;
            });

            $(document).on('click', '.edit_specialty-item-btn', function(event) {
                var id = $(this).closest('tr').data('id');
                location.href = '<?php echo e(env('APP_URL')); ?>/mgr/<?php echo e($controller??''); ?>/edit_specialty/' + id;
            });
            ...
            $(document).on('click', '.del_academics-item-btn', function(event) {
                ...
                $.ajax({
                    type: "POST",
                    url: '<?php echo e(env('APP_URL')); ?>/mgr/<?php echo e($controller??''); ?>/del_academics',
                    data: {
                        'id': id
                    },
                    dataType: "json",
            ...
            $(document).on('click', '.del_specialty-item-btn', function(event) {
                ...
                $.ajax({
                    type: "POST",
                    url: '<?php echo e(env('APP_URL')); ?>/mgr/<?php echo e($controller??''); ?>/del_specialty',
                    data: {
                        'id': id
                    },
                    dataType: "json",
            ...
            $(document).on('click', '.add-specialty-item-btn', function(event) {
                var id = $(this).closest('tr').data('id');
                location.href = '<?php echo e(env('APP_URL')); ?>/mgr/<?php echo e($controller??''); ?>/add_specialty/' + id;
            });
